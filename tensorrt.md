...
> 环境变量
>
> 将TensorRT目录下的 `bin` 和 `lib` 目录添加到环境变量, 添加 `bin` 目录主要是为了使用 `trtexec`

> 安装python库
>
> 分别进入 `python` `graphsurgeon` `onnx-graphsurgeon` 安装对应的包

> 测试
>
> 进入python环境,使用如下命令,不报错说明成功
...
```powershell
trtexec --onnx=resnet18.onnx --saveEngine=resnet18.engine     	          	# Precision: FP32
trtexec --onnx=resnet18.onnx --saveEngine=resnet18.engine --fp16	        # Precision: FP32+FP16
trtexec --onnx=resnet18.onnx --saveEngine=resnet18.engine --int8	        # Precision: FP32+INT8
trtexec --onnx=resnet18.onnx --saveEngine=resnet18.engine --fp16 --int8	  	# Precision: FP32+FP16+INT8
trtexec --onnx=resnet18.onnx --saveEngine=resnet18.engine --best	        # Precision: FP32+FP16+INT8
```
